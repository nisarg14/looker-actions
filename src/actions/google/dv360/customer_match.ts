...
import { DV360ActionRequest } from "./lib/dv_request";

const LOG_PREFIX = "[DV360 Customer Match]"

export class DV360CustomerMatch
    extends Hub.OAuthAction
    implements UseGoogleOAuthHelper {

    /******** Core action properties ********/

    readonly name = "dv360_customer_match";
    readonly label = "DV360 Customer Match";
    readonly iconName = "google/dv360/dv360_icon.svg";
    readonly description = "Upload data to DV360 Customer Match.";
    readonly supportedActionTypes = [Hub.ActionType.Query];
    readonly supportedFormats = [Hub.ActionFormat.Json];
    ...
    async execute(hubReq: Hub.ActionRequest) {
        ...
        try {
            const adsRequest = await DV360ActionRequest.fromHub(hubReq, this, log);
            await adsRequest.execute(hubReq);
    ...
    async form(hubReq: Hub.ActionRequest) {
        ...
        const log = this.makeLogger(hubReq.webhookId);
        try {
            const dvWorker = await DV360ActionRequest.fromHub(hubReq, this, log)
            wrappedResp.form = await dvWorker.makeForm();
            return wrappedResp.returnSuccess(dvWorker.userState);
...
if (process.env.DV360_CLIENT_ID
    && process.env.DV360_CLIENT_SECRET
    && process.env.DV360_DEVELOPER_TOKEN
) {
    const gacm = new DV360CustomerMatch(
